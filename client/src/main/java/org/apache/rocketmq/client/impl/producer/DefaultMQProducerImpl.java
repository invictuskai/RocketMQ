...
/**
 * 通过Request方法发送的消息，生产者现车个在这个Request发送之后会挂起等待，等到消费者消费完这条消息之后要给对应的生产者发送一个响应消息，响应消息被生产者收到之后对应的生产者线程会恢复继续运行。
 * 生产者msg添加了额外信息，客户端ID，关联ID。
 * 发送到broker之后，消费者从broker拿到这条消息检查消息类型发现是一个需要发送响应的request消息，消费完之后根据msg的关联id，客户端id生成一个相应消息，发送给brrker
 * broker拿到这条响应消息之后根据客户端id找到对应的Channel，将响应消息发送给生产者。
 * 生产者这把拿到响应消息之后读取出来关联id找到对应的requestFuture，将对应阻塞的线程唤醒，相当于模拟了一次RPC调用过程。
 */
// DefaultMQProducerImpl是一个生产者，生产者要注册到客户端实例上
public class DefaultMQProducerImpl implements MQProducerInner {
    private final InternalLogger log = ClientLogger.getLog();

    // 生成invokeId，打印日志使用，无实际意义
    private final Random random = new Random();

    // DefaultMQProducer信息，这里用来获取client配置
    private final DefaultMQProducer defaultMQProducer;

    // topic主题发布信息映射表  key-主题，value-主题发布信息
    ...
    // 生产者当前状态， 默认为CREATE_JUST
    private ServiceState serviceState = ServiceState.CREATE_JUST;

    // 客户端实例对象，生产者启动之后需要注册到该客户端实例对象中，观察者模式
    ...
    // 异步消息发送线程池，如果指定的话，就不再选择使用默认的defaultAsyncSenderExecutor
    ...
    // 参数：startFactory 默认为true
    public void start(final boolean startFactory) throws MQClientException {

        //默认当前生产者serviceState为CREATE_JUST
        switch (this.serviceState) {
            case CREATE_JUST:

                // 首先设置当前生产者为serviceState为START_FAILED
                this.serviceState = ServiceState.START_FAILED;

                // 检查相关的配置
                this.checkConfig();
                // 如果当前producerGroup不是CLIENT_INNER_PRODUCER_GROUP
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    // 设置当前生产者实例为当前进程PID+时间戳
                ...
                // 将当前defaultMQProducer的默认topic放入到主题发布数据映射表中
                this.topicPublishInfoTable.put(this.defaultMQProducer.getCreateTopicKey(), new TopicPublishInfo());

                // 条件满足，启动MQClientInstance
                if (startFactory) {
        ...
        //当前客户端实例向所有的broker节点发送心跳
        this.mQClientFactory.sendHeartbeatToAllBrokerWithLock();

        // 开启RequestFuture定时任务处理，处理过期的RequestFuture
    ...
    private void checkConfig() throws MQClientException {
        ...
        //producerGroup不能为DEFAULT_PRODUCER_GROUP
    ...
    /**
     * 在消息发送过程中，可能会多次执行选择消息队列这个方法，lastBrokerName就是上一次选择的执行发送消息失败的Broker。
     * 第一次执行消息队列选择时，lastBrokerName为null，此时直接用sendWhichQueue自增再获取值，与当前路由表中消息队列的个数取
     * 模，返回该位置的MessageQueue.
     * 如果消息发送失败，下次进行消息队列选择时规避上次MessageQueue所在的Broker，否则有可能再次失败。
     * Broker不可用后，路由信息中为什么还会包含该Broker的路由信息呢？
     * 首先，NameServer检测Broker是否可用是有延迟的，最短为一次心跳检测间隔（10s）；
     * 其次，NameServer不是检测到Broker宕机后马上推送消息给消息生产者，而是消息生产者每隔30s更新一次路由信息，因此消息生产者最快感知Broker最新的路由信息也需要30s。
     * 这就需要引入一种机制，在Broker宕机期间，一次消息发送失败后，将该Broker暂时排除在消息队列的选择范围中。
     * @param tpInfo 主题发布信息
     * @param lastBrokerName 上一次发送失败的broker
     * @return 可以发送消息的MessageQueue
     */
     ...
     *
     * @param msg 消息
     * @param communicationMode 消息发送方法
    ...
    private SendResult sendDefaultImpl(
        ...
        // 获取到当前topic对应的主题发布信息并且MessageQueue存在
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 发送总的尝试次数  同步发送模式下为1+2=3，异步发送模式下为1次
            int timesTotal = communicationMode == CommunicationMode.SYNC ? 1 + this.defaultMQProducer.getRetryTimesWhenSendFailed() : 1;

            // 当前是第几次发送
            int times = 0;

            // 创建一个数组，记录对应次数发送使用的brokerName
            ...
            for (; times < timesTotal; times++) {
                ...
                // 上次发送的brokerName，第一次发送的时lastBrokerName为null
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // 计算本轮发送耗时，如果耗时已经超过timeOut，直接不发送消息跳出循环
                        ...
                        switch (communicationMode) {
                            ...
                            case ONEWAY:
                                ...
                                // 如果是同步发送
                            case SYNC:
                                // 如果发送结果不是发送成功，说明服务端broker本条调消息存储失败
                                if (sendResult.getSendStatus() != SendStatus.SEND_OK) {

                                    // 如果当前消息未存储成功，是否选择其他broker进行重试
            ...
            // 执行到这里，有三种情况
            // 1，循环尝试发送几次之后都没能成功
            // 2，sendKernelImpl执行失败，sendResult为null
            // 3，发送超时


            // 条件满足，说明循环尝试发送几次之后都没能成功
            if (sendResult != null) {
                return sendResult;
            }
    ...
    private SendResult sendKernelImpl(final Message msg,
         ...
         * 则从NameServer主动更新topic的路由信息。如果路由更新后还是找不到Broker信息，则抛出MQClientException，提示Broker不存在，
         */
        long beginStartTime = System.currentTimeMillis();
        // 根据brokerName获取对应broker的master节点的主机地址
        ...
        // 创建SendMessageContext，记录当前发送消息的上下文
        SendMessageContext context = null;
        if (brokerAddr != null) {

            // 如果客户端开起来VIP通道的话，broker地址的端口是vip通道的端口号。
            // VIP端口和普通端口在服务端创建NioSocketChannel提供的线程池不是同一个
            ...
            try {
                ...
                // 扩展点，注册checkForbiddenHook用来控制消息发送
                ...
                // 用户扩展点，执行msgHook.before方法，
                if (this.hasSendMessageHook()) {
                    // 记录消息发送的上下文信息，用于后面执行扩展点
                ...
                // 执行sendMessageHook的after方法，即使消息发送过程中出现异常，也会执行after方法
                if (this.hasSendMessageHook()) {
